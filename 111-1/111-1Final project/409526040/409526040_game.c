...
int main(){
    ...
    system("C:\\Users\\MOU\\OneDrive\\Desktop\\CCU.png");//output image
...
int booster_decrease_record( int *booster_slot,int *booster_owned, int *booster_have,int *booster_record,int type){
    ...
    for (int k = temp; k < *booster_slot;k++){
        booster_record[k] = booster_record[k + 1];// move each sequence history booster forward one block which also means getting rid of the sequence num which previous recording
    }
    (*booster_have)--;
    booster_owned[type]--;
