...
void selling_hotdog(int *money, int *price,int *start, int *booster_owned,int *booster_record);
void lottery(int *money, int *price, int *booster_slot, int *booster_owned,int *booster_record);

//function for the map system
void map_control(int *money,int *booster_owned,int *booster_record);
void map_print(int i, int j,int *b,int *money,int *booster_owned,int *booster_record);

//function for the aircraft game
void aircraft_main(int *money, int *booster_slot, int *booster_owned,int *day,int *booster_record);
...
int main(){
    ...
    int booster_owned[3] = {0, 0, 1},booster_record[booster_slot];    
    int start = 1;
    ...
    while (start == 1)
    {
        selling_hotdog(&money, &price,&start, booster_owned,booster_record);
        if(start==2)break;
        lottery(&money, &price,&booster_slot, booster_owned,booster_record);
        aircraft_main(&money,&booster_slot, booster_owned,&day,booster_record);  
    }
...
void selling_hotdog(int *money, int *price,int *start, int *booster_owned,int *booster_record){
    ...
    while (booster_switch != 4)
    {
        ...
        if (booster_switch == 1)
        {
            if (booster[0] == 0)//if close
            {
                ...
                booster_owned[0]--;//due to using it minus one
            }
            else//if open
            {
                booster[0] = 0;//turn off it
                booster_owned[0]++;//due to not using it plus 1
            }
        }
        if (booster_switch == 2)
        {
    ...
    if(booster_owned[0]==-1){//if playper doesent own the booster，close it,and plus the value
        booster[0] = 0;
        booster_owned[0]++;
    }
    ...
    *money += total_earn;
    ...
    while(check_selection!=(5+booster[2])){//area open 6 out,not open 5 out
        ...
        if(result[check_selection]==1){
            map_control(&*money, booster_owned,booster_record);
...
void lottery(int *money, int *price,int *booster_slot, int *booster_owned,int *booster_record){
    ...
    int selection;
    int booster_have = 0, i = 0;//i use to record booster getting sequence
...
void map_control(int *money,int *booster_owned,int *booster_record){
    ...
    do{
        ...
        map_print(y, x, b,money,booster_owned,booster_record);
...
//must confirm player have the booster
int booster_decrease_record( int *booster_slot,int *booster_owned, int *booster_have,int *booster_record,int type){
    int temp=-1;
    for (int i = 0; i < *booster_slot;i++){
